...
## 1. Objective
Replace the cramped radial ring with a full-width **Bottom Gauge Bar** integrated directly into the "GÜN" (Day) HUD pill. The 2-minute (120 seconds) day progression is displayed as a crisp, high-visibility 3px progress bar spanning the entire width of the pill, accompanied by a clean square icon box that dynamically transitions between the 4 time-of-day phases (Sabah, Öğle, Akşam, Gece).
...
- **4 Time-of-Day Phases (120s Timeline)**:
  1. **Phase 1: Sabah (000s - 030s / 0.00 - 0.50 dk)**: `#FFB703` (Gold) • `Icons.wb_sunny_rounded`
  2. **Phase 2: Öğle (030s - 071s / 0.50 - 1.18 dk)**: `#38BDF8` (Sky Blue) • `Icons.storefront_rounded`
  3. **Phase 3: Akşam (071s - 104s / 1.18 - 1.73 dk)**: `#F97316` (Sunset Orange) • `Icons.wb_twilight_rounded`
  4. **Phase 4: Gece (104s - 120s / 1.73 - 2.00 dk)**: `#818CF8` (Midnight Indigo) • `Icons.nightlight_round`
...
- Unit and widget tests in `test/day_progress_ring_test.dart`:
  ...
  3. Phase visuals switch at 30s, 71s, 104s, 120s.
...
- [ ] At 2 minutes (120s), the day counter increments with a tactile pulse and the gauge bar resets smoothly.
